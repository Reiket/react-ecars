...
import {HeaderMenu} from "./types/menu.types";
import {MdKeyboardArrowDown} from "react-icons/md";
import {MenuEnums, ShipItem} from "../../../types/header.types";
import {cn} from "../../../../../../../shared/utils";
import MenuList from "./components/MenuList";

const Menu: React.FC<HeaderMenu> = React.memo(({filters, type, onClickToFilters, isBurger, items, filter, text}) => {
        ...
        return (
            <div ref={ref} className={itemClassNames}>
                ...
                {MenuEnums.selectShip === type && items[filter] && <img width={20} height={20} src={(items[filter] as ShipItem).img} alt="Flags"/>}

                <div className={cn("header-menu", {
                    "header-menu_nav": isBurger,
                })}>
                    <>
                        {MenuEnums.selectShip === type && items[filter] ? (items[filter] as ShipItem).country : items[filter]}
                    </>
